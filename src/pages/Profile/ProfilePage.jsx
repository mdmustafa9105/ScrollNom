...
export const ProfilePage = () => {
  ...
  // Video File Upload & Storage State
  ...
  const handleCreateReel = async (e) => {
    ...
    try {
      ...
      // 2. Create content database record linked to creator, restaurant, and dish
      const contentRes = await fetch(`${API_BASE}/content`, {
        ...
        body: JSON.stringify({
          ...
          posterUrl: storedVideoUrl,
          caption: newReelTitle,
          dishTitle: newReelTitle,
          dishPrice: 350,
          restaurantName: selectedCollab
        })
      });

      if (!contentRes.ok) {
        throw new Error('Failed to save Nommly Reel content record.');
      }
      ...
      // Refresh creator video list
      await fetchMyVideos();
    } catch (err) {
      console.error('[CREATOR REEL UPLOAD ERROR]', err);
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-brand-cream rounded-3xl p-6 shadow-floating border border-brand-cream-dark relative animate-slide-up space-y-4">
            <div className="flex items-center justify-between border-b border-brand-cream-dark pb-3">
              <div className="flex items-center space-x-2">
                <Video className="w-5 h-5 text-brand-coral" />
                <h3 className="text-base font-extrabold text-brand-charcoal">Upload Nommly Reel</h3>
              </div>
              <button
                onClick={() => { setShowCreateModal(false); setSelectedVideoFile(null); }}
            ...
            <form onSubmit={handleCreateReel} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-brand-charcoal-muted mb-1">Dish Video Title</label>
                <input
                  type="text"
                  required
                  placeholder="e.g. Crispy Birria Tacos Tasting & Review! 🌮"
                  value={newReelTitle}
              ...
              <div>
                ...
                >
                  ...
                  <option value="The Smashed Patty Co.">The Smashed Patty Co.</option>
                  <option value="Taco Haven">Taco Haven</option>
                </select>
              </div>

              {/* Real Video File Picker Input */}
              <input
                type="file"
                ref={fileInputRef}
                ...
                {selectedVideoFile ? (
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] font-bold text-emerald-600">
                      {(selectedVideoFile.size / (1024 * 1024)).toFixed(2)} MB • Ready for Upload
                    </p>
                  </div>
                ) : (
                  <>
              ...
              >
                {uploading ? `Uploading Video (${uploadProgress}%)...` : 'Publish Nommly Reel & Start Earning'}
              </button>
            </form>
          </div>
        </div>
